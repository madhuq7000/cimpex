// features/auth/product/pages/ProductDetails.tsx

import { useEffect, useState } from "react";
import { useParams } from "react-router-dom";

import { getProductByIdApi } from "../productApi";
import type { Product } from "../types";

export default function ProductDetails() {
  const { id } = useParams();

  const [product, setProduct] = useState<Product | null>(null);

  // ✅ selected large image
  const [selectedImage, setSelectedImage] = useState("");

  useEffect(() => {
    if (id) {
      getProductByIdApi(id).then((res) => {
        const productData = res.data;

        setProduct(productData);

        // ✅ default first image
        if (productData.images?.length > 0) {
          setSelectedImage(productData.images[0]);
        }
      });
    }
  }, [id]);

  if (!product) {
    return (
      <div className="container py-5">
        <h2>Loading...</h2>
      </div>
    );
  }

  return (
    <div className="container">
      <div className="row" style={{ marginLeft: "0px", marginRight: "0px" }}>
        {/* ========================= */}
        {/* LEFT SIDE - IMAGE GALLERY */}
        {/* ========================= */}
        <div className="col-md-6" style={{ marginLeft: "0px !important" }}>
          {/* LARGE IMAGE */}
          <div className="border rounded shadow-sm overflow-hidden mb-3">
            <img
              src={`http://localhost:3000/uploads/${selectedImage}`}
              alt={product.name}
              className="img-fluid w-100"
              style={{
                height: "500px",
                objectFit: "cover",
              }}
            />
          </div>

          {/* THUMBNAILS */}
          <div className="d-flex gap-2 flex-wrap">
            {product.images?.map((img, index) => (
              <div
                key={index}
                onClick={() => setSelectedImage(img)}
                className={`border rounded overflow-hidden ${
                  selectedImage === img
                    ? "border-primary border-3"
                    : "border-secondary"
                }`}
                style={{
                  width: "90px",
                  height: "90px",
                  cursor: "pointer",
                }}
              >
                <img
                  src={`http://localhost:3000/uploads/${img}`}
                  alt={`thumb-${index}`}
                  className="w-100 h-100"
                  style={{
                    objectFit: "cover",
                  }}
                />
              </div>
            ))}
          </div>
        </div>

        {/* ========================= */}
        {/* RIGHT SIDE - PRODUCT INFO */}
        {/* ========================= */}
        <div className="col-md-6" style={{ marginRight: "0px !important" }}>
          {/* PRODUCT NAME */}
          <h1 className="mb-3">{product.name}</h1>

          {/* PRICE */}
          <h2 className="text-success mb-4">₹ {product.price}</h2>

          {/* DESCRIPTION */}
          <div className="mb-4">
            <h4>Description</h4>

            <p className="text-muted">{product.description}</p>
          </div>

          {/* BUTTONS */}
          <div className="d-flex gap-3">
            <button className="btn btn-primary btn-lg">Add To Cart</button>

            <button className="btn btn-outline-dark btn-lg">Buy Now</button>
          </div>
        </div>
      </div>
    </div>
  );
}
